...
# Abstract
The human liver and stomach represent two distinct filtering systems with complementary functions in the body. While the liver filters blood and performs metabolic detoxification, the stomach filters ingested materials through physical and chemical processes. This study aimed to characterize the transcriptomic differences between these organs to better understand their specialized functions. RNA-sequencing was performed on three biological replicates each of human liver and stomach tissues. Following quality assessment, adapter trimming, and alignment to the human reference genome (GRCh38), differential gene expression analysis was conducted using DESeq2. Principal component analysis revealed extreme separation along PC1 (explaining 95% of variance), confirming fundamentally different expression profiles between tissues. Top differentially expressed genes included liver-specific markers involved in detoxification pathways (ALB, SERPINC1, CYP enzymes) and stomach-specific genes related to mucosal protection (MUC5AC, GKN1, TFF1). Hierarchical clustering demonstrated two distinct gene expression patterns with minimal overlap between tissues. These findings support our hypothesis that liver and stomach tissues exhibit distinctive transcriptomic signatures reflecting their specialized filtering functions, with liver expressing higher levels of detoxification and metabolic pathway genes, while stomach shows elevated expression of genes related to acid production, mucus secretion, and digestive processes.
...
## Mapping
The trimmed sequences were then aligned to the human genome (GRCh38) using STAR version 2.7.11b. The mapping results were evaluated to assess the quality of the alignment and the overall mapping rate. SAMtools version 1.19.2 was then used to create the .bam files that will be used for counting.
...
#### STRING
Protein-protein interaction networks for the top 50 differentially expressed genes were constructed using STRING database v12.0 (https://string-db.org). The top 50 DE genes (25 liver-specific, 25 stomach-specific) were searched with both a minimum required interaction score of $0.400$ and a high minimum required interaction score of $0.700$.

#### Cytoscape
Cytoscape v3.9.1 was used to visualize the protein-protein interaction networks generated from STRING. The networks were imported into Cytoscape, and various layout algorithms were applied to optimize the visualization of interactions. The networks were then annotated with gene names, fold changes, and p-values to highlight the most significant interactions.
Network visualizations were exported as both vector format images and Cytoscape session files.
...
```{r f, eval=FALSE, fig.height=9, fig.width=6.5, message=FALSE, warning=FALSE, include=TRUE}
...
Blood coagulation was the most significantly enriched process (26 genes, 6.6%, p=5.7E-21), followed by xenobiotic metabolic process (23 genes, 5.8%, p=1.2E-15), proteolysis (34 genes, 8.6%, p=9.2E-8), and innate immune response (24 genes, 6.1%, p=6.1E-4). These enriched pathways clearly reflect the liver's multifaceted filtering functions, including blood component regulation, detoxification, protein processing, and immunological surveillance.

```{r fig6_enrichBP, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
...
```{r fig7_enrichCC, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
# Create a bar plot for the top 10 enriched biological processes
enrich_bp <- enrichment_results %>%
...
```{r fig8_enrichMF, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
...
The most significantly enriched pathway was metabolic pathways (130 genes, 32.8%, p=5.54E-32), highlighting the liver's central role in metabolism. This was followed by complement and coagulation cascades (36 genes, 9.1%, p=5.57E-31), which aligns with the liver's function in blood protein homeostasis.
Particularly notable was the enrichment of pathways directly related to the liver's detoxification role, including bile secretion (24 genes, 6.1%, p=1.04E-15), drug metabolism - cytochrome P450 (19 genes, 4.8%, p=2.87E-12), and metabolism of xenobiotics by cytochrome P450 (17 genes, 4.3%, p=1.13E-09).

```{r fig9_enrichKEGG, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
...
ggplot(enrich_kegg, aes(x = reorder(Term, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 10 Enriched KEGG Pathways | Figure 7",
       x = "KEGG Pathway",
       y = "Gene Count") +
  theme_minimal()
```
...
### Interpro
```{r fig9_enrichInterpro, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
...
ggplot(enrich_interpro, aes(x = reorder(Term, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 10 Enriched Interpro | Figure 9",
       x = "Interpro",
       y = "Gene Count") +
  theme_minimal()
```

In contrast to liver tissue, stomach-specific genes showed enrichment in biological processes related to epithelial integrity, gastric acid secretion, and mucus production (Table X). Key enriched pathways included digestion, mucosal protection, and ion transport, reflecting the stomach's role as a food processing filter. Genes such as MUC5AC, GKN1, and TFF1 were central to these stomach-specific functions, highlighting the functional specialization between these two filtering organs.

In conclusion, these enrichment analysis results provide compelling evidence supporting our hypothesis. The liver's transcriptomic profile reveals a sophisticated blood and chemical filtering system, with highlighted expression of genes involved in detoxification, protein processing, lipid metabolism, and hemostasis. In contrast, the stomach's gene expression pattern reflects its specialized role in food processing through acid production, mucus secretion, and initial digestive functions. These distinct molecular signatures underscore the specialized nature of these organs' filtering mechanisms, validating our initial hypothesis about their functional differentiation.

### Network Analysis
```{r fig10_network, echo=FALSE, fig.height=10, fig.width=8, message=FALSE, warning=FALSE}
setwd("~/Documents/binf694/R_projects/BINF694_MainProject")
knitr::include_graphics("Project 1/finalDeliverables/results/string_normal_image400.png")
```

There are several key observations from the network analysis. Firstly, the network has a highly interconnected cluster with a large amount of blood proteins, aligning with that of the liver's filtering function. This cluster includes genes such as APOH, SERPIND1, and F2, which are involved in blood coagulation and detoxification processes. The presence of these genes in the network highlights the liver's role in maintaining blood homeostasis and detoxifying harmful substances. These result are similar to that of the enrichment analysis as GO:0007596~blood coagulation presented as the most significant biological process in our top 400 differentially expressed genes.

Another functional module identified in the network is the alcohol dehydrogenase family identified by genes: ADH4, ADH6, ADH1A all contributing to alcohol metabolism. This module is particularly relevant to the liver's detoxification function, as it plays a crucial role in metabolizing alcohol and other xenobiotics. The presence of these genes in the network further supports the liver's specialized filtering role in processing and detoxifying various substances.

**Verify**
Edge coloring indicates multiple lines of evidence supporting these interactions, with many connections validated by experimental data (magenta edges) and database annotations (blue edges).
...
\newpage
# Discussion
This study set out to test the hypothesis that liver and stomach tissues would show differentially expressed gene patterns reflecting their specialized filtering functions, with the liver expressing higher levels of detoxification and metabolic pathway genes, while the stomach would show elevated expression of genes related to acid production, mucus secretion, and initial digestive processes. Our transcriptomic analysis has revealed distinct molecular signatures that strongly support this hypothesis and provide novel insights into the specialized filtering mechanisms of these organs.

## Enrichment Results
Our enrichment analysis results showed a notable predominance of liver-related pathways and functions compared to stomach-specific processes. This asymmetry likely reflects the fundamental biological complexity difference between these two filtering organs. The liver serves as the body's central metabolic hub and primary detoxification center, necessitating a diverse array of molecular machinery encompassing multiple metabolic pathways, detoxification systems, blood protein regulation, and immune functions. This is evidenced by the extensive enrichment in pathways including xenobiotic metabolism, complement and coagulation cascades, lipid metabolism, and drug processing. In contrast, the stomach's filtering function is more specialized and focused on acid production, mucus secretion, and initial food processing, requiring a comparatively narrower set of molecular pathways. Additionally, the liver's position as the body's primary interface with bloodborne substances requires a more complex transcriptional program to manage the filtering of diverse endogenous and exogenous compounds, while the stomach primarily filters ingested materials through relatively fewer specialized cellular processes. The greater diversity and significance of liver-enriched pathways in our results underscores the liver's more multifaceted filtering role compared to the more specialized but equally essential filtering function of the stomach.

## Methodological Evaluation
The use of three biological replicates for each tissue type provided statistical robustness for our differential expression analysis, as evidenced by the clear separation in PCA analysis and the high statistical significance of our enrichment results. The application of DESeq2 for differential expression analysis, combined with comprehensive pathway analysis using DAVID and Enrichr, allowed for a thorough characterization of the functional differences between these tissues.
...
Secondly, while our analysis focused on the top differentially expressed genes, it is important to consider that other genes with lower fold changes may also play significant roles in liver and stomach functions. Future studies could benefit from a more comprehensive analysis of the entire transcriptome, including low-abundance transcripts and non-coding RNAs.

## Future Direction
Claude 3.7 Sonnet[https://claude.ai/share/883da6f3-b64f-428b-8a11-4c270111f1e7]

Several promising directions emerge from this study. First, expanding the analysis to include additional filtering organs—such as kidneys and lungs—would provide a more comprehensive understanding of specialized filtering mechanisms across the body.
